...
def commutator_compilation_qcirc(c, A, B, tau, ppart):
    """
...
def linear_plus_commutator_compilation_qcirc(b, c, A, B, tau, ppart):
    """
...
def UC_qcirc(c1, c2, c3, A, B, tau, ppart):
    ...
    if c2 != 0 and c1 == 0 == c3:
        return commutator_compilation_qcirc(c2, A, B, tau, ppart)
    ...
    if c3 == 0 and c1 != 0 != c2:
        return linear_plus_commutator_compilation_qcirc(c1, c2, A, B, tau, ppart)
        # below is an alternative approach
        #a, b = c2/4, 1
...
def CPF_symp_qcirc(order, A, B, tau, ppart, reps=1):
    ...
    where PF is a product formula of order 1 or 2 and C is the associated symplectic corrector.
    """
    #tau = t/reps
    if order == 1:
...
def data_CPF_symp_qcirc_error(order, A, B, time_ticks, ppart, nsteps):

    error_data = [CPF_symp_qcirc_error(order, A, B, t, ppart, nsteps) for t in time_ticks]
    return error_data